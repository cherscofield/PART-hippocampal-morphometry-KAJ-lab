...
cbar.ax.tick_params(labelsize=18)       # ⬅ Colorbar tick font size
...
plt.tight_layout()
plt.savefig("rh.Cluster Permutation_result_custom_rb_masked.png", dpi=300)
plt.show()
...
def plot_Cluster Permutation_with_effect_size(T_obs, sign_mask, title, save_name):
    from matplotlib.colors import LinearSegmentedColormap
    import matplotlib.pyplot as plt
    import numpy as np
    global label_matrix
...
d_map1, M1, title1, file1 = run_Cluster Permutation_comparison(df, 0, 1, "(TDP+ vs TDP−)", "rh.Cluster Permutation_tdp1_vs_tdp0.png")
plot_Cluster Permutation_with_effect_size(d_map1, M1, title1, file1)

# Comparison 2: TDP− (0) vs Control (2)
d_map2, M2, title2, file2 = run_Cluster Permutation_comparison(df, 0, 2, "(Control vs TDP−)", "rh.Cluster Permutation_tdp2_vs_tdp0.png")
plot_Cluster Permutation_with_effect_size(d_map2, M2, title2, file2)

# Comparison 3: TDP+ (1) vs Control (2)
d_map3, M3, title3, file3 = run_Cluster Permutation_comparison(df, 1, 2, "(Control vs TDP+)", "rh.Cluster Permutation_tdp2_vs_tdp1.png")
plot_Cluster Permutation_with_effect_size(d_map3, M3, title3, file3)
...
masks = {
    "Cluster Permutation_PART_vs_Control": sig_mask,
...
for key, mask in subfield_masks.items():
    ...
    # Save VTK
    vtk_label = f"Cluster Permutation_TDPneg_vs_TDPpos_Subfield{key}"
    ...
    output_vtk_path = f"rh.mid-surface_Cluster Permutation_subfield{key}.vtk"
    ...
    plt.tight_layout()
    plt.savefig(f"rh.Cluster Permutation_hedgesg_tdp0_vs_tdp1_subfield{key}.png", dpi=300)
    plt.show()
...
def run_braak_Cluster Permutation_comparison(df_subset, braak_a, braak_b, label, save_name_prefix):
    group0, group1 = [], []
...
def plot_Cluster Permutation_with_effect_size(T_obs, sig_mask, title, save_name):
    flipped_T_obs = T_obs.T
    flipped_mask = sig_mask.T
    masked_T = np.ma.masked_where(~flipped_mask, flipped_T_obs)
...
for braak_stage in [1,2,3,4]:
    ...
    save_prefix = f"rh.Cluster Permutation_braak0_vs_{braak_stage}_tdpneg"
...
d_map_braak12, sig_mask_braak12 = run_part_braak_Cluster Permutation_comparison(
    ...
    save_prefix="rh.Cluster Permutation_part_notdp_braak12_vs_controls"
)
...
if d_map_braak12 is not None and sig_mask_braak12 is not None:
    plot_Cluster Permutation_with_effect_size(
        ...
        "rh.Cluster Permutation_part_notdp_braak12_vs_controls.png"
    )
    save_vtk_with_mask(base_mesh, sig_mask_braak12, "rh.Cluster Permutation_part_notdp_braak12_vs_controls")
...
d_map_braak34, sig_mask_braak34 = run_part_braak_Cluster Permutation_comparison(
    ...
    save_prefix="rh.Cluster Permutation_part_notdp_braak34_vs_controls"
)
...
if d_map_braak34 is not None and sig_mask_braak34 is not None:
    plot_Cluster Permutation_with_effect_size(
        ...
        "rh.Cluster Permutation_part_notdp_braak34_vs_controls.png"
    )
    save_vtk_with_mask(base_mesh, sig_mask_braak34, "rh.Cluster Permutation_part_notdp_braak34_vs_controls")
