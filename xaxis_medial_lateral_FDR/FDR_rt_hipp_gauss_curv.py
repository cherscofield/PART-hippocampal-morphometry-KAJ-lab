...
ax.set_title("Significant Differences (PART vs Control) in Gaussian Curvature", fontsize=10)
...
plt.savefig"path/to/your/data_file.csv"  # UPDATE: Replace with your data path
plt.close()

# === Per-subject average Gaussian curvature ===
...
def analyze_gauss_curv_tdp_groups(df, group_a, group_b, label_a, label_b, output_prefix):
    ...
    c = ax.imshow(significance_map, cmap="bwr", interpolation="none", origin="lower",
                  extent=[0, x_max, 0, y_max])
    ax.set_title(f"Significant Differences ({label_a} vs {label_b}) in Gaussian Curvature", fontsize=10)
    ax.set_xlabel("X axis (Medial to Lateral)")
    ax.set_ylabel("Y axis (Posterior to Anterior)")
    fig.colorbar(c, ax=ax, label=f"t-statistic ({label_a} - {label_b})")
    plt.savefig(f"path/to/your/data_file.csv"  # UPDATE: Replace with your data path)
    plt.close()

    # Per-subject average over significant points
    significant_coords = [coords[i] for i, sig in enumerate(rejected) if sig]
    results = []
    for mrn in df["mrn"].unique():
        subj_data = df[df["mrn"] == mrn]
    ...
    print(f"\n--- Average Gaussian Curvature in Significant Regions ({label_a} vs {label_b}) ---")
...
analyze_gauss_curv_tdp_groups(df, 1, 2, "TDP+", "Controls", "TDPpos_vs_Control")
analyze_gauss_curv_tdp_groups(df, 0, 2, "TDP-", "Controls", "TDPneg_vs_Control")

# === Export significant coordinates to CSV ===
signif_list = []
...
signif_df.to_csv"path/to/your/data_file.csv"  # UPDATE: Replace with your data path, index=False
print("Exported significant coordinates to CSV.")
...
# === RIGHT HEMISPHERE GAUSSIAN CURVATURE ===
