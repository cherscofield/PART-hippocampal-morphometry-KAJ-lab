"""
Right Hemisphere Gaussian Curvature Analysis with FDR Correction
...
# Plot PART vs Control
fig, ax = plt.subplots(figsize=(10, 6))
c = ax.imshow(significance_map, cmap="bwr", interpolation="none", origin="lower",
              extent=[0, x_max, 0, y_max])
ax.set_title("Significant Differences (PART vs Control) in RH Gaussian Curvature (FDR)", fontsize=10)
...
plt.savefig(f"{OUTPUT_DIR}rh_gauss_PART_vs_Control_FDR.png", dpi=300)
plt.close()

# Per-subject average in significant regions
...
for mrn in df["mrn"].unique():
    ...
    if values:
        group = subj_data["group_PART_vs_control"].iloc[0]
        results.append((mrn, group, np.mean(values)))

results_df = pd.DataFrame(results, columns=["mrn", "group", "gauss_curvature"])
part_group = results_df[results_df["group"] == 1]["gauss_curvature"]
control_group = results_df[results_df["group"] == 0]["gauss_curvature"]

print("\n--- Average Gaussian Curvature in Significant Regions ---")
print(f"PART (n={len(part_group)}): mean = {part_group.mean():.5f}, std = {part_group.std():.5f}")
print(f"Control (n={len(control_group)}): mean = {control_group.mean():.5f}, std = {control_group.std():.5f}")

# =============================================================================
# TDP GROUP PAIRWISE COMPARISONS
# =============================================================================
def analyze_gauss_curv_tdp_groups(df, group_a, group_b, label_a, label_b, output_prefix):
    """Perform FDR-corrected t-tests between TDP groups."""
    ...
    c = ax.imshow(significance_map, cmap="bwr", interpolation="none", origin="lower",
                  extent=[0, x_max, 0, y_max])
    ax.set_title(f"Significant Differences ({label_a} vs {label_b}) in RH Gaussian Curvature", fontsize=10)
    ax.set_xlabel("X axis (Medial to Lateral)")
    ax.set_ylabel("Y axis (Posterior to Anterior)")
    fig.colorbar(c, ax=ax, label=f"t-statistic ({label_a} - {label_b})")
    plt.savefig(f"{OUTPUT_DIR}rh_gauss_{output_prefix}_FDR.png", dpi=300)
    plt.close()
...
# Run TDP comparisons
analyze_gauss_curv_tdp_groups(df, 0, 1, "TDP-", "TDP+", "TDPneg_vs_TDPpos")
analyze_gauss_curv_tdp_groups(df, 1, 2, "TDP+", "Control", "TDPpos_vs_Control")
analyze_gauss_curv_tdp_groups(df, 0, 2, "TDP-", "Control", "TDPneg_vs_Control")

# Export significant coordinates
signif_list = []
for idx, sig in enumerate(rejected):
    if sig:
        x, y = coords[idx]
        signif_list.append({
            "grid_x": x,
            ...
            "p_value_FDR": pvals_corrected[idx]
        })

signif_df = pd.DataFrame(signif_list)
signif_df.to_csv(f"{OUTPUT_DIR}rh_gauss_significant_coords.csv", index=False)
print("\nExported significant coordinates to CSV.")

# =============================================================================
# LINE PLOTS ALONG AXES
# =============================================================================
def plot_gauss_curv_line_by_axis(df, axis='x', group_col='tdp_status',
...
# Y-axis comparisons
plot_gauss_curv_line_by_axis(df, axis='y', group_col='tdp_status',
    group_a=0, group_b=1, label_a="TDP-", label_b="TDP+",
    output_path=f"{OUTPUT_DIR}rh_gauss_yaxis_TDPneg_vs_TDPpos.png")

plot_gauss_curv_line_by_axis(df, axis='y', group_col='tdp_status',
    group_a=1, group_b=2, label_a="TDP+", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_yaxis_TDPpos_vs_Control.png")

plot_gauss_curv_line_by_axis(df, axis='y', group_col='tdp_status',
    group_a=0, group_b=2, label_a="TDP-", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_yaxis_TDPneg_vs_Control.png")

plot_gauss_curv_line_by_axis(df, axis='y', group_col='group_PART_vs_control',
    group_a=1, group_b=0, label_a="PART", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_yaxis_PART_vs_Control.png")

# X-axis comparisons
plot_gauss_curv_line_by_axis(df, axis='x', group_col='tdp_status',
    group_a=0, group_b=1, label_a="TDP-", label_b="TDP+",
    output_path=f"{OUTPUT_DIR}rh_gauss_xaxis_TDPneg_vs_TDPpos.png")

plot_gauss_curv_line_by_axis(df, axis='x', group_col='tdp_status',
    group_a=1, group_b=2, label_a="TDP+", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_xaxis_TDPpos_vs_Control.png")

plot_gauss_curv_line_by_axis(df, axis='x', group_col='tdp_status',
    group_a=0, group_b=2, label_a="TDP-", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_xaxis_TDPneg_vs_Control.png")

plot_gauss_curv_line_by_axis(df, axis='x', group_col='group_PART_vs_control',
    group_a=1, group_b=0, label_a="PART", label_b="Control",
    output_path=f"{OUTPUT_DIR}rh_gauss_xaxis_PART_vs_Control.png")
