...
#for mrn in df["mrn"].unique():
 ...
 #       print(f"⚠️ Skipping subject {mrn} — no data")
 #       continue
...
X1, Y1 = np.meshgrid(part_grid.columns[::-1], part_grid.index)
Z1 = part_grid[part_grid.columns[::-1]].values
...
X2, Y2 = np.meshgrid(control_grid.columns[::-1], control_grid.index)
Z2 = control_grid[control_grid.columns[::-1]].values
...
plt.tight_layout()
output_path_comparison = "path/to/your/data_file.csv"Replace with your data path
...
X0, Y0 = np.meshgrid(tdp0_grid.columns[::-1], tdp0_grid.index)
Z0 = tdp0_grid[tdp0_grid.columns[::-1]].values
...
X1, Y1 = np.meshgrid(tdp1_grid.columns[::-1], tdp1_grid.index)
Z1 = tdp1_grid[tdp1_grid.columns[::-1]].values
...
X2, Y2 = np.meshgrid(tdp2_grid.columns[::-1], tdp2_grid.index)
Z2 = tdp2_grid[tdp2_grid.columns[::-1]].values
...
plt.tight_layout()
output_path_tdp = "path/to/your/data_file.csv"Replace with your data path
...
######################################################################################################
#Perform a per-point (x, y) comparison of hippocampal thickness between PART (1) and Control (0) groups using a two-sample t-test.
#Apply False Discovery Rate (FDR) correction to the resulting p-values.
#Then, generate a surface plot that:
#Highlights points with statistically significant differences after FDR correction
#Optionally shows the direction of effect (e.g., PART > Control)
# Results
...
c = ax.imshow(np.fliplr(significance_map), cmap="bwr", interpolation="none", origin="lower",
              extent=[max(df_long["x_index"]), 0, 0, 20])
...
# Save the plot
output_path_sigmap = "path/to/your/data_file.csv"Replace with your data path
...
##########################################################################################
#Build a per-subject average thickness across the significant (x, y) points.
#1 Extract the (x, y) coordinates where FDR-corrected p-values are significant
#2 Compute the average thickness across those points for each subject
#3 Separate the results into PART and Control groups
#4 Print their group-level means and standard deviations

# === Step 1: Get (x, y) coordinates that were significant ===
...
################################################################################################################
# === TDP Status Analysis: Prompt ===
# I want to perform per-point (x, y) statistical comparisons of thickness values 
# between TDP status groups using a two-sample t-test, followed by FDR correction 
# to identify statistically significant differences.

# I need two pairwise group comparisons:
# 1. TDP 0 vs TDP 1
# 2. TDP 1 vs TDP 2

# For each comparison, the following steps should be performed:

# ✅ Statistical Testing:
# - Perform a two-sample t-test at each (x, y) coordinate across subjects in the two groups
# - Apply False Discovery Rate (FDR) correction to control for multiple comparisons

# ✅ Visualization:
# - Generate a 2D surface map highlighting the (x, y) points where thickness differs significantly after FDR correction
# - Use a color scale (e.g., bwr) to show the direction of the difference:
#     - Red = group A > group B
#     - Blue = group B > group A

# ✅ Quantification:
# - For each subject, calculate the average thickness across only the significant (x, y) points
# - Compute and print group-level mean and standard deviation for these regions
# - Clearly report which group shows thicker regions on average

# === Function: Run TDP group comparison with FDR, plot, and thickness summary ===
...
# === Run TDP comparisons ===
analyze_tdp_groups(df_long, 0, 1, "PART without TDP", "PART with TDP", "TPART without TDP_vs_PART with TDP")
analyze_tdp_groups(df_long, 1, 2, "TDP 1", "Controls", "TDP_1_vs_Controls")
...
def plot_thickness_line_by_axis(df_long, axis='x', group_col='tdp_status',
    ...
    plt.title(f"{label_a} vs {label_b}: LH Thickness Along {'Medial-Lateral' if axis == 'x' else 'Posterior-Anterior'} Axis")
    plt.legend()
    plt.tight_layout()
    if axis == 'x':
...
plot_thickness_line_by_axis(df_long, axis='y', group_col='tdp_status',
...
plot_thickness_line_by_axis(df_long, axis='y', group_col='PART=1_control=0',
    group_a=1, group_b=0, label_a="PART", label_b="Control", 
    output_path="path/to/your/data_file.csv"  # UPDATE: Replace with your data path)


# === X-axis (Medial to Lateral), flipped ===
plot_thickness_line_by_axis(df_long, axis='x', group_col='tdp_status',
    group_a=0, group_b=1, label_a="TDP-", label_b="TDP+", 
    output_path="path/to/your/data_file.csv"  # UPDATE: Replace with your data path)
...
plot_thickness_line_by_axis(df_long, axis='x', group_col='PART=1_control=0',
    group_a=1, group_b=0, label_a="PART", label_b="Control", 
    output_path="path/to/your/data_file.csv"  # UPDATE: Replace with your data path)
